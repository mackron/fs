...
fs_file_open(pFS, "assets/file.txt", FS_READ | FS_OPAQUE, &pFile);</code></pre><br><br>Here the archive is mounted to the virtual path "assets". Because the path "assets/file.txt" is prefixed with "assets", the file system knows to look inside the mounted archive without having to search for it. |
